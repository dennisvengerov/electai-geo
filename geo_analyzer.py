...
class GEOAnalyzer:
    ...
    async def analyze_company_visibility(
        ...
        if progress_callback:
            progress_callback(0, num_queries + 1,
        ...
        if progress_callback:
            progress_callback(
                1, num_queries + 1,
                f"Generated {len(queries)} queries. Starting analysis...")
        ...
        if progress_callback:
            progress_callback(num_queries + 1, num_queries + 1,
                              f"Analysis completed in {total_time:.1f}s")
        ...
        return {
            ...
            total_time,
            'query_results':
            [self._query_result_to_dict(qr) for qr in query_results],
            'summary':
            summary
        }
        ...
        try:
            response = await client.chat.completions.create(
                ...
                messages=[{
                    ...
                    "system",
                    "content":
                    "You are a helpful assistant conducting web research. "
                    ...
                    "user",
                    "content":
                    f"Research and answer this query comprehensively: {query}"
                }],
                temperature=1)
    ...
    def _analyze_results(self, query_results: List[QueryResult],
        ...
        positions = [
            qr.position for qr in cited_results if qr.position is not None
        ]
        ...
        for competitor in all_competitors:
            competitor_counts[competitor] = competitor_counts.get(
                competitor, 0) + 1
        ...
        execution_times = [
            qr.execution_time for qr in query_results if qr.execution_time > 0
        ]
        ...
        return {
            ...
            len([
                ...
                if not qr.response.startswith('Error:')
            ])
        }
